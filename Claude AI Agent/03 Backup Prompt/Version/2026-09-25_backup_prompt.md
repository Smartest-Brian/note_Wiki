# AI Agent 重建備份（Prompt B）— 2026-09-25

> 本檔案由「Prompt A」（`dev/init-prompt/generate_backup_prompt.md`）觸發產生，
> 依 workspace 主規則檔「八、備份與重建機制」規格，重新掃描當下實際檔案系統
> 狀態後產生。上一版備份：`dev/init-prompt/backups/2026-09-23_backup_prompt.md`
> （本版不覆寫，各自獨立保留）。

本檔案目的：即使這台 VM 完全損毀，只憑本檔案內容，即可在全新環境重建目前
這個 AI agent 的規則、目錄結構、skill、memory 累積的心得與 dev/ 下的工具
程式碼（不含任何機密或可重抓資料，見下方「十、排除項清單」）。
...
4. `.claude/memory/` 完整內容
5. `.claude/wrappers/` 完整內容
6. `dev/` 各專案原始碼與套件安裝方式
7. `~/.claude/skills/` 個人 skill 完整內容
8. `~/Library/LaunchAgents/` workspace plist 完整內容
9. `README.md`
10. 排除項清單
11. 重建後續步驟 Checklist
...
.claude/CLAUDE.md
.claude/memory
.claude/memory/dev.md
.claude/memory/general.md
.claude/memory/stock.md
.claude/wrappers
...
dev
dev/gmaps-lookup
dev/gmaps-lookup/.tabelog_state
...
dev/gmaps-lookup/node_modules
dev/gmaps-lookup/package-lock.json
dev/gmaps-lookup/package.json
dev/gmaps-lookup/queries.json
dev/gmaps-lookup/tabelog_reviews.js
dev/gmaps-lookup/tabelog_urls.json
dev/gmaps-lookup/tabelog_utils.js
dev/init-prompt
dev/init-prompt/backups
dev/init-prompt/generate_backup_prompt.md
dev/outbox-cleanup
dev/outbox-cleanup/cleanup_outbox.py
dev/stock-chart
dev/stock-chart/.venv
dev/stock-chart/__pycache__
dev/stock-chart/kline_chart.py
dev/stock-fomo
dev/stock-fomo/.venv
dev/stock-fomo/__pycache__
dev/stock-fomo/backtest_fomo.py
dev/stock-fomo/fomo_index.py
dev/stock-model
dev/stock-model/.venv
dev/stock-model/__pycache__
...
dev/stock-sector
dev/stock-sector/.venv
dev/stock-sector/__pycache__
...
dev/telegram-notify
dev/telegram-notify/.venv
dev/telegram-notify/__pycache__
dev/telegram-notify/telegram_notify.py
logs
outbox
```

> 註：`data/{topic}/`、`logs/`、`outbox/`、`dev/init-prompt/backups/`（歷史備份快照）
> 內容不逐檔列出，屬於可重抓/累積型/一次性資料，詳見「十、排除項清單」。

---

## 二、`~/.claude/CLAUDE.md` 完整內容
...
## 三、`~/claude-workspace/.claude/CLAUDE.md` 完整內容
...
### 查詢類請求的預設行為
使用者說「幫我查詢 {主題}」「查一下 {主題}」這類單純查詢/比較用語，
且未特別指定要不要查社群、要不要記錄時，預設行為為：
1. 同時執行一般網路搜尋（WebSearch）與社群搜尋（Threads / IG，
   套用社群貼文搜尋的既有流程），彙整成一份摘要或比較表回覆。
2. 純屬「查詢彙整」性質、沒有要求存檔或傳送的這類任務，
   **不適用**上方「紀錄義務」：不寫入 logs/activity_log.jsonl，
   也不寫入 memory/{topic}.md。
3. 若使用者當次額外要求「存成檔案」「傳給我」「幫我記住這個」等，
   則該次改回「紀錄義務」的一般規則正常記錄。
4. 使用者可在單次請求中明確覆蓋預設值（例如「這次不用查社群」
   「這個要記錄」），以當次明確指示為準，本節只是沒特別說明時
   的預設值，不是固定死的規則。
...
---
分類: 程式撰寫與除錯（跨主題）
最後更新: 2026-09-23（新增備份/重建機制 Prompt A + Prompt B，取代舊版 claude_init_prompt.md）
---
...
| init-prompt | dev/init-prompt/ | general（整個 AI agent 的災難復原備份機制） | Markdown（非可執行程式碼） | **2026-09-23 由「單版本 claude_init_prompt.md」改版為「Prompt A + 依日期版本化 Prompt B」機制，詳細規格見 workspace CLAUDE.md「八、備份與重建機制」**。`generate_backup_prompt.md`（Prompt A）：穩定、可重複下達的產生指令，貼給任何已載入本 workspace CLAUDE.md 的 Claude session 就能觸發重新掃描並產生最新的 Prompt B；只在產生流程邏輯本身變動時才需要修改。`backups/{YYYY-MM-DD}_backup_prompt.md`（Prompt B）：每次產生一份新的日期版本快照，不覆寫舊版本，完整嵌入兩份 CLAUDE.md、5 個 memory 檔案、wrapper script、當時 dev/ 底下 8 個專案（telegram-notify/outbox-cleanup/stock-news/stock-chart/stock-model/stock-fomo/stock-sector/gmaps-lookup）全部原始碼、3 個個人 skill 的 SKILL.md、4 個 launchd plist、README，並排除 activity log/可重抓歷史資料/venv與node_modules/密鑰明文等（詳見 Prompt B 內「排除項與理由」章節）。首版 `2026-09-23_backup_prompt.md` 共 6246 行、約 311KB。舊檔 `claude_init_prompt.md` 已刪除，其涵蓋範圍已被新機制完整取代並擴充。 |
...
- 2026-09-25：依使用者要求，刪除 hardware、travel 兩個主題的全部資料
  （memory/hardware.md、memory/travel.md、data/hardware/、data/travel/，
  含原本10/16-10/20東京行程規劃內容；CLAUDE.md「二、主題分類機制」主題
  索引表移除這兩列；activity_log.jsonl 移除22筆相關舊紀錄），使用者確認
  過即將出發的東京行程也一併刪除，事後沒有備份。
...
### 6.1 `dev/outbox-cleanup/`

**相依套件安裝方式：** 純標準庫，系統 `python3` 即可執行，不需 venv。
...
### 6.2 `dev/telegram-notify/`

**相依套件安裝方式：** Python venv（3.12）。相依套件：`fastmcp`。
  建立方式：`python3.12 -m venv .venv && .venv/bin/pip install fastmcp`
...
**相依套件安裝方式：** Python venv（3.12）。相依套件見 `requirements.txt`（pandas, pyarrow）。
  建立方式：`python3.12 -m venv .venv && .venv/bin/pip install -r requirements.txt`
...
### 6.4 `dev/gmaps-lookup/`

**相依套件安裝方式：** Node.js。相依套件見 `package.json`（playwright）。
  建立方式：`npm install`（`package-lock.json` 為產生檔，不隨備份嵌入，`npm install` 會依 `package.json` 重新產生）。首次需 `npx playwright install`。
...
### 6.5 `dev/stock-sector/`

**相依套件安裝方式：** Python venv（3.12）。相依套件：pandas, numpy, scipy, matplotlib（無 requirements.txt，依程式 import 推斷）。
  建立方式：`python3.12 -m venv .venv && .venv/bin/pip install pandas numpy scipy matplotlib`
...
**相依套件安裝方式：** Python venv（3.12）。相依套件：pandas, numpy, scipy, scikit-learn（無 requirements.txt，依程式 import 推斷）。
  建立方式：`python3.12 -m venv .venv && .venv/bin/pip install pandas numpy scipy scikit-learn`
...
**相依套件安裝方式：** Python venv（3.12）。相依套件：pandas, matplotlib, mplfinance（無 requirements.txt，依程式 import 推斷）。
  建立方式：`python3.12 -m venv .venv && .venv/bin/pip install pandas matplotlib mplfinance`
...
**相依套件安裝方式：** 純文件，無程式相依。（`backups/` 底下的歷史版本快照不重複嵌入本備份。）
...
> **本次備份排除的 dev/ 檔案**（依「一律排除」判斷為單次查詢暫存輸入 / 快取狀態 / 產生檔，非工具本體）：
> - `dev/gmaps-lookup/queries.json`、`dev/gmaps-lookup/tabelog_urls.json`　— 某次查詢用的候選店家清單，屬單次查詢暫存輸入檔
> - `dev/gmaps-lookup/.tabelog_state/cooldown.json`　— 執行期冷卻狀態快取
> - `dev/gmaps-lookup/package-lock.json`　— npm 產生鎖定檔，`npm install` 會依 `package.json` 重新產生，等同 pip freeze 輸出不需附上
> - `dev/init-prompt/backups/*.md`　— 歷史備份快照本身（含本檔），不遞迴嵌入自身

---

## 七、`~/.claude/skills/` 個人 skill 完整內容
...
> 註：`~/.claude/skills/synced/` 底下為 Anthropic 內建 skill 的同步副本（deep-research、docx、pdf、pptx、xlsx、skill-creator、morning、import-memory、docs、built-in-browser、computer-use 等），非使用者個人建立，隨 Claude Code 更新即會自動重新同步，不屬於本備份範圍。

---

## 八、`~/Library/LaunchAgents/` workspace plist 完整內容
...
## 十、排除項清單（一律不寫入本檔，也不得要求使用者貼明文）

| 項目 | 原因 | 重建方式 |
|------|------|----------|
| `logs/activity_log.jsonl`、`logs/archive/` | 累積型操作日誌，無保留價值 | 重建後重新開始記錄即可 |
| `data/{topic}/` 底下歷史資料（股價、新聞 parquet、universe 快取等） | 可重抓 | 用 `dev/` 對應腳本重新抓取 |
| `outbox/` | 一次性產出，本來就會 3 天自動清除 | 不需重建 |
| 各專案 `.venv/`、`node_modules/`、`__pycache__/`、`.tabelog_state/cache` | 可由套件管理器重建或屬暫存快取 | 依上方各專案「套件安裝方式」重新安裝 |
| Keychain 內密鑰明文（`telegram-bot-token`、`telegram-chat-id`） | 機密，不落地 | 使用者在新機器手動於 Keychain 重新輸入（見下方 Checklist） |
| `claude_desktop_config.json` | 本來就只在使用者確認後才建立，且不含明文 | 依主規則「五、Token 安全儲存機制」5.4 節知識參考重建 |
| 訓練/模型用大型資料檔、螢幕截圖、`.DS_Store` | 系統雜項 / 可重新產生 | 不需重建 |
| `dev/gmaps-lookup/queries.json`、`tabelog_urls.json` | 單次查詢用暫存輸入檔，非工具本體 | 依需求重新產生查詢清單 |
| `dev/gmaps-lookup/.tabelog_state/cooldown.json` | 執行期冷卻狀態快取 | 程式執行時自動重新產生 |
| `dev/gmaps-lookup/package-lock.json` | npm 產生鎖定檔 | `npm install` 依 `package.json` 重新產生 |
| 歷史備份快照（`dev/init-prompt/backups/*.md`，含本檔自身） | 避免遞迴嵌入、檔案無限膨脹 | 不需重建；每次備份都是獨立快照 |

---

## 十一、重建後續步驟 Checklist

1. **建目錄**：在新環境建立 `~/claude-workspace/`，並依「一、目錄結構總覽」建立
   `.claude/{memory,wrappers}/`、`data/`、`dev/`、`logs/{archive/}`、`outbox/`、
   `dev/init-prompt/backups/` 等子目錄。
2. **寫入兩份 CLAUDE.md**：
   - `~/.claude/CLAUDE.md`（內容見「二」）
   - `~/claude-workspace/.claude/CLAUDE.md`（內容見「三」）
3. **寫入 memory**：`~/claude-workspace/.claude/memory/{dev,general,stock}.md`（內容見「四」）。
4. **寫入 wrappers 並 `chmod +x`**：
   `~/claude-workspace/.claude/wrappers/run_telegram_mcp.sh`（內容見「五」），
   接著 `chmod +x ~/claude-workspace/.claude/wrappers/run_telegram_mcp.sh`。
5. **依專案建立 venv / npm 並安裝套件**：依「六」各專案小節列出的「相依套件安裝方式」逐一執行
   （`python3.12 -m venv .venv && .venv/bin/pip install ...`，或 `npm install`）。
6. **寫入 dev/ 原始碼**：將「六」內逐檔嵌入的程式碼寫回對應相對路徑。
7. **寫入個人 skill**：`~/.claude/skills/{jp-restaurant-finder,jp-travel-planner,social-post-search}/SKILL.md`
   （內容見「七」）。
8. **建立 launchd plist 並 `launchctl bootstrap`**：將「八」的 4 份 plist 寫回
   `~/Library/LaunchAgents/`，再逐一執行
   `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.claudeworkspace.<name>.plist`。
9. **寫入 README.md**（內容見「九」）。
10. ⚠️ **提醒使用者手動在 Keychain 建立密鑰**（不得由 Claude 代為輸入明文）：
    ```
    security add-generic-password -a "$USER" -s "telegram-bot-token" -w "<token>"
    security add-generic-password -a "$USER" -s "telegram-chat-id" -w "<chat id>"
    ```
11. ⚠️ **確認後才建立 `claude_desktop_config.json`**：依主規則 5.4 節內容，且需先
    向使用者確認才寫入，指向 wrapper script 而非明文 token。
12. ⚠️ **端對端測試 Telegram 發送前，先向使用者確認測試訊息內容**，確認後再發送。
13. **重新抓取可重抓資料**（依需求，非必要立即執行）：股價歷史、新聞、universe 快取等，
    用 `dev/stock-*`、`dev/stock-news` 對應腳本重新抓取到 `data/{topic}/`。
14. **寫回 `activity_log.jsonl` / `memory/dev.md`**：記錄本次重建完成的時間與涵蓋範圍。
15. **向使用者回報完成狀態**：列出已重建項目、待使用者手動完成的項目（Keychain、
    `claude_desktop_config.json`、端對端測試）。
